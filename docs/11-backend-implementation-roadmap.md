...
La baseline applicativa `3cf48902b6c193c5f25740eab7e774ce26e3dcc3` ha completato `clean verify` con 50 suite, 312 test, 0 failure, 0 error e un solo skipped MySQL opt-in. Il test opt-in è stato successivamente eseguito su MySQL 8.0.44 con 1 test, 0 failure, 0 error e 0 skipped. Il backend espone 29 endpoint applicativi; `/error` è un fallback tecnico separato. Il Wrapper Maven, la CI su Ubuntu/Windows, la configurazione tipizzata, Flyway, UTC, autorizzazioni scoped, privacy, email verification/SMTP, Booking storico e il contratto uniforme degli errori risultano chiusi per il perimetro MVP.
...
Il database originale è stato controllato soltanto in lettura ed è rimasto invariato: 22 tabelle, 181 colonne, history assente e conteggi originari. Il backend applicativo è pronto per il trasferimento controllato e lo sviluppo frontend può iniziare. L'avvio del backend trasferito sull'originale deve però avvenire soltanto durante una finestra di migrazione autorizzata, seguendo il runbook di `10-database-schema.md`; non usare `flyway repair`, modifiche manuali della history o avvio automatico prima della baseline.
