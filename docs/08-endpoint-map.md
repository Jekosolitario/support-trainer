...
> Contratto Booking V1 corrente: `PATCH .../reject` richiede `{ "reason": "..." }`; `PATCH .../cancel` accetta body assente/`{}`/reason nullable ma la richiede per ogni cancellazione `CONFIRMED`. `cancelledBy` è server-side. Tutte le mutation sono ammesse soltanto per `now < MAX(items.scheduledEnd)` e restituiscono `409 BOOKING_REQUEST_ENDED` a fine esatta o oltre. Il detail include metadata decisionali nullable. Specifica completa: [19-booking-domain-contract-v1.md](19-booking-domain-contract-v1.md).
...
- Il detail aggiunge `rejectionReason`, `cancellationReason` e `cancelledBy`, nullable per compatibilità con lo storico pre-V9.
...
La conferma usa la fine snapshot `MAX(items.scheduledEnd)`: è ammessa durante `IN_PROGRESS` e rifiutata quando `now` raggiunge o supera tale fine, senza dipendere dallo stato live dello slot.
...
- il body `{ "reason": "..." }` è obbligatorio; reason viene trimmata ed è lunga al massimo 1000 caratteri
- `rejectedAt` e `rejectionReason` vengono valorizzati e la response restituisce il dettaglio completo
...
- Client `PENDING` può omettere reason; ogni cancel `CONFIRMED` richiede reason
- l'actor non arriva dal payload: il server assegna `CLIENT` o `PROFESSIONAL`
- la mutation richiede `now < MAX(items.scheduledEnd)`
...
Ogni cancellazione consentita valorizza atomicamente `cancelledAt`, `cancellationReason` e `cancelledBy`, conserva un eventuale `confirmedAt` già presente e restituisce il dettaglio completo. I null rimangono ammessi in lettura per record legacy pre-V9.
